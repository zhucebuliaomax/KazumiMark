...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "Kazumi Max");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "Kazumi Max");
  }
  ...
  fl_engine_get_binary_messenger(fl_view_get_engine(view)), "app.kazumimax/intent", FL_METHOD_CODEC(codec));
  ...
  self->storage_method_channel = fl_method_channel_new(
      fl_engine_get_binary_messenger(fl_view_get_engine(view)), "app.kazumimax/storage", FL_METHOD_CODEC(codec));
  fl_method_channel_set_method_call_handler(
